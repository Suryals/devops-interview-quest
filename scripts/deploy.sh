#!/usr/bin/env bash
# Build the single-file bundle and deploy to Cloudflare Pages.
set -euo pipefail
cd "$(dirname "$0")/../app"
npx vite build
node -e '
...
cd ..
mkdir -p site && cp index.html site/
npx --yes wrangler pages deploy site --project-name devops-interview-quest --commit-dirty=true
